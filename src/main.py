...
def notify_expiring_issues():
    """
    Monitora le issue in scadenza o scadute e invia notifiche aggregate.
    Garantisce l'invio alle mail_aggiuntive anche se gli assegnatari sono "anonimi".
    """
    ...
    # Recupero destinatari sempre attivi dallo YAML
    ...
    for item in issues:
        try:
            ...
            # Dati della issue da inserire nel report
            issue_info = {
            ...
            # 1. Assegnatari con email pubblica
            assignees = issue_data.get('assignees', {}).get('nodes', [])
            assignee_emails = [a['email'] for a in assignees if a.get('email')]
            
            # 2. Unione con mail_aggiuntive (usiamo un set per evitare duplicati)
            recipients = set(assignee_emails + extra_emails)

            if not recipients:
                logger.debug(f"Issue #{issue_data['number']} ignorata: nessun destinatario email disponibile.")
                continue
    ...
    if notifications_map:
        send_aggregated_emails(notifications_map, "Riepilogo Scadenze Progetto")
    else:
        logger.info("Nessuna notifica da inviare dopo i filtri.")


def notify_missing_duedate():
    """
    Monitora le issue senza data di scadenza e invia notifiche aggregate.
    """
    notifications_map = defaultdict(list)
    extra_emails = [e.strip() for e in getattr(config, 'mail_aggiuntive', '').split(',') if e.strip()]
    ...
    for email, items in notifications_map.items():
        ...
        <html>
            <body style='font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;'>
                ...
                <p>Ciao, le seguenti attività nel progetto <b>{getattr(config, 'project_name', 'Project')}</b> richiedono attenzione:</p>
